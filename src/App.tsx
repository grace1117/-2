...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeProperties = onSnapshot(propertiesRef, async (snapshot) => {
      if (snapshot.empty) {
        // Database is clean and empty, let's proactively seed it with standard listings
        ...
        snapshot.forEach((snapDoc) => {
          loaded.push(snapDoc.data() as Property);
        });
  ...
  useEffect(() => {
    ...
    const unsubscribeReviews = onSnapshot(reviewsRef, async (snapshot) => {
      if (snapshot.empty) {
        // Seed default qualitative feedbacks for better immersive demonstration
        console.log("Seeding comments database for property: ", selectedPropertyId);
        try {
          const defaultWenshanReviews = [
            { id: 'r1', author: 'Lin J.', dept: '世新資傳', avatar: 'LJ', content: 'Quiet for studying and the soundproofing is actually decent. Highly recommend for finals week.', rating: 4.8, userId: 'system' },
            { id: 'r2', author: 'Chen Y.', dept: '世新傳管', avatar: 'CY', content: 'Landlord is very responsive. Fixed my AC within 24 hours. Safe building for female students.', rating: 4.9, userId: 'system' }
          ];
          const defaultOtherReviews = [
            { id: 'r3', author: 'Wang S.', dept: '世新資管', avatar: 'WS', content: '整體環境不錯，雖然是頂加但熱水器與水壓很新很熱，租金真的佛心！', rating: 4.5, userId: 'system' }
          ];
          const syncList = selectedPropertyId === 'prop-wenshan-premium' ? defaultWenshanReviews : defaultOtherReviews;
          for (const item of syncList) {
            await setDoc(doc(db, 'properties', selectedPropertyId, 'reviews', item.id), {
              ...item,
              createdAt: serverTimestamp()
            });
          }
        } catch (err) {
          console.error("Feedback seeding error: ", err);
        }
      } else {
